                ...
                for j in range(len(centroids)):
                    if i != j:
                        ...
                        self.assertTrue(distance_to_centroid <= dst,
                                        ...
                                        + str(distance_to_centroid) + "; distance to centroid of cluster" + str(
                                            j) + ": " + str(dst) + "own centroid : " + str(currentCluster.getCentroid())
                                        + " the other centroid " + str(
                                            kMeanClusterer.getCluster(j).getCentroid()) + " the value : " + str(
                                            obs) + "nb obs tot " + str(
                                            len(kMeanClusterer.getCluster(0).getObservations()) + len(
                                                kMeanClusterer.getCluster(1).getObservations()) + len(
                                                kMeanClusterer.getCluster(2).getObservations())))
